...
Read& Read::trim(const SequenceInfo& seq_info, const trim_direction& td,
    ...
    std::string_view left_trimmed_seq{};
    std::string_view right_trimmed_seq{};
    ...
    if (td.trim_top5end) {
        ...
        myUtility::smith_waterman(target_5end_view, seq_info.m_top5end_query, align_config, align_5end_result);
        if (align_5end_result.get_percent(seq_info.m_top5end_query) > get<1>(seq_info.m_top5end) &&
            align_5end_result.get_identity() > get<2>(seq_info.m_top5end)) {
            // auto [query_5start, target_5start] = align_result_top5end.get_start_idx();
            auto [query_5end, target_5end] = align_5end_result.get_stop_idx();
            if (target_5end >= sequence_view.size()) {
                // target_5end will never is bigger than sequence_view.size();
                m_sequence = "";
                m_quality = "";
                return *this;
            }
            trim_start_idx = target_5end;
            left_trimmed_seq = sequence_view.substr(0, target_5end);
        ...
        if (td.trim_top3end) {
            ...
            myUtility::smith_waterman(target_3end_view, seq_info.m_top3end_query, align_config, align_3end_result);
            if (align_3end_result.get_percent(seq_info.m_top3end_query) > get<1>(seq_info.m_top3end) &&
                align_3end_result.get_identity() > get<2>(seq_info.m_top3end)) {
                auto [query_3start, target_3start] = align_3end_result.get_start_idx();
                auto [query_3end, target_3end] = align_3end_result.get_stop_idx();
                if (target_3end >= sequence_view.size()) {
                    m_sequence = "";
                    m_quality = "";
                    return *this;
                }
                right_trimmed_seq = sequence_view.substr(sequence_view.size() - target_3start);
                trim_end_idx = m_sequence.size() - target_3start; // the bases number should be trimmed from read right
            }
        }
        if (!left_trimmed_seq.empty() || !right_trimmed_seq.empty()) {
            std::osyncstream{log} << fmt::format("{0}\t{1}\t{2}\n",
                                                 m_id,
                                                 left_trimmed_seq.empty() ? "None" : left_trimmed_seq,
                                                 right_trimmed_seq.empty() ? "None" : right_trimmed_seq) << '\n';
            if (!left_trimmed_seq.empty()) {
                cout << align_5end_result.to_string(seq_info.m_top5end_query.size());
            }
            if (!right_trimmed_seq.empty()) {
                cout << align_3end_result.to_string(seq_info.m_top3end_query.size());
            }
        }
    }


    if (td.trim_bot5end && !left_trimmed_seq.empty() && !right_trimmed_seq.empty()) {
        // check whether this read is reversed complemented or not
        AlignmentResult align_5end_result{true};
        AlignmentResult align_3end_result{false};
        std::string_view target_5end_view{
        ...
        myUtility::smith_waterman(target_5end_view, seq_info.m_bot5end_query, align_config, align_5end_result);
        if (align_5end_result.get_percent(seq_info.m_bot5end_query) > get<1>(seq_info.m_bot5end) &&
            align_5end_result.get_identity() > get<2>(seq_info.m_bot5end)) {
            // auto [query_5start, target_5start] = align_5end_result.get_start_idx();
            auto [query_5end, target_5end] = align_5end_result.get_stop_idx();
            if (target_5end >= sequence_view.size()) {
                m_sequence = "";
                m_quality = "";
            ...
            trim_start_idx = target_5end;
            left_trimmed_seq = sequence_view.substr(0, trim_start_idx);
            sequence_view.remove_prefix(trim_start_idx);
        }

        if (td.trim_bot3end) {
            // check where this read is reversed complemented or not and check read 3end
            std::string_view target_3end_view{
                sequence_view.size() > get<0>(seq_info.m_bot3end)
                    ? sequence_view.substr(sequence_view.size() - get<0>(seq_info.m_bot3end))
                    : sequence_view
            };
            myUtility::smith_waterman(target_3end_view, seq_info.m_bot3end_query, align_config, align_3end_result);
            if (align_3end_result.get_percent(seq_info.m_bot3end_query) > get<1>(seq_info.m_bot3end) &&
                align_3end_result.get_identity() > get<2>(seq_info.m_bot3end)) {
                auto [query_3start, target_3start] = align_3end_result.get_start_idx();
                auto [query_3end, target_3end] = align_3end_result.get_stop_idx();
                if (target_3end >= sequence_view.size()) {
                    m_sequence = "";
                ...
                right_trimmed_seq = sequence_view.substr(sequence_view.size() - target_3start);
                trim_end_idx = m_sequence.size() - target_3start;
            }
        }
        if (!left_trimmed_seq.empty() || !right_trimmed_seq.empty()) {
            std::osyncstream{log} << fmt::format("{0}\t{1}\t{2}\n",
                                                 ...
                                                 right_trimmed_seq.empty() ? "None" : right_trimmed_seq) << '\n';
            if (!left_trimmed_seq.empty()) {
                cout << align_5end_result.to_string(seq_info.m_bot5end_query.size());
            }
            if (!right_trimmed_seq.empty()) {
                cout << align_3end_result.to_string(seq_info.m_bot3end_query.size());
            }
        }
    }
