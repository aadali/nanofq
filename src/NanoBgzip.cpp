...
namespace nanobgzip
{
    namespace
    {
            ...
            size_t written_bytes)
        {
            ...
            gz_header header;
            header.extra_len = 8;
            // set the xlen is 8 bytes => 1 byte sid1, 1 byte sid2 , 2 bytes slen, 4 bytes block size
            header.text = 0;
            header.hcrc = 0;
            header.name = nullptr;
            ...
            int init_ret = deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 16 + MAX_WBITS, 8,
                                        Z_DEFAULT_STRATEGY);
            if (init_ret != Z_OK) {
        ...
        unsigned need_uncompressed_size)
    {
        ...
        nanobgzip::NanoBgzipHeader header; // sizeof(header) == 20
