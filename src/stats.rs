...
use crate::fastq2::{FastqRecord, RecordEachStats, chunk_records_from_fastq, stats_one_fastq};
...
fn stats_one_file(
    ...
    if thread == 1 {
        stats_one_fastq(fastq_file, use_dorado_q, use_gc)
    } else {
...
fn stats_fastq_dir(
    ...
    if thread == 1 {
        fastqs
            .into_iter()
            .map(|x| stats_one_fastq(x.to_str().unwrap(), use_dorado_q, use_gc))
            .flatten()
            .collect()
    } else {
        fastqs
...
pub fn run_stats(stats_cmd: &ArgMatches) {
    ...
    let all_stats = match input_t {
        ...
        InputType::OneFastqFile | InputType::OneFastqGzippedFile => {
            stats_one_file(input_file, *thread as usize, use_dorado_q, use_gc, *chunk)
        }
