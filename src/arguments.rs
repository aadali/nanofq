...
pub fn parse_arguments() -> ArgMatches {
    ...
    let subseq_cmd = Command::new("subseq")
        ...
        .arg(Arg::new("input")
            ...
            .action(ArgAction::Set)
            .required(true)
            .help("A fastq[.gz] or indexed bam file used to search sub fastq records")
        )
        .arg(Arg::new("output")
            .short('o')
            .long("output")
            .action(ArgAction::Set)
            .help("Output uncompressed fastq file")
        )
        .arg(Arg::new("names")
            .short('n')
            .long("names")
        ...
        .arg(Arg::new("names_file")
            .short('N')
            .long("names_file")
            .action(ArgAction::Set)
            .help("Read names list file, one read name per line")
        )
        .arg(Arg::new("region")
            .short('r')
            .long("region")
            .action(ArgAction::Set)
            .help("Interested region in indexed bam, format should be: ContigStr:StartPosition-EndPosition, with 0-based half-open intervals, multi region can be separated by comma")
        )
        .arg(Arg::new("bed")
            .short('L')
            .long("bed")
...
you can choice one from [LSK, RAD, ULK, RBK, PCS, PCB, NBD_1, NBD_2, ..., NBD_95, NBD_96]. NBD_{number} means kit name with barcode number.")
            ...
            .arg(Arg::new("end5_align_pct_rc")
                    ...
                    .default_value("0.8")
                    .value_parser(|x: &str| positive_number_parse::<f64>(x, "--end5_align_pct", true, 0.0f64, 1.0f64)))
            .arg(Arg::new("end5_align_ident_rc")
            ...
            .arg(Arg::new("end3_align_pct_rc")
                    ...
                    .default_value("0.8")
                    .value_parser(|x: &str| positive_number_parse::<f64>(x, "--end5_align_pct", true, 0.0f64, 1.0f64)))
            .arg(Arg::new("end3_align_ident_rc")
    ...
    let amplicon_cmd = Command::new("amplicon")
            ...
            .arg(Arg::new("len")
                    ...
                    .help("estimated amplicon length,  more accurate, more better."))
            .arg(Arg::new("range")
                    .short('R')
                    .long("range")
                    .action(ArgAction::Set)
                    .default_value("10")
                    .value_parser(|x: &str| positive_number_parse::<u8>(x, "--range", false, 1, 100))
                    .help("reads whose length in range of this value percent of estimate length will be used"))
            .arg(Arg::new("mafft")
                    .long("mafft")
                    .action(ArgAction::Set)
                    .default_value("mafft")
    ...
    let cmd = Command::new("nanofq")
        .version("0.3.0")
        .about("A simple program for nanopore long reads to stats, subseq, filter, trim...")
