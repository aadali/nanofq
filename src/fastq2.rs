...
fn read_one_fastq(fastq_file: &str, need_description: bool) -> Vec<FastqRecord> {
...
pub fn stats_one_fastq(fastq_file: &str, use_dorado_q: bool, use_gc: bool) -> Vec<RecordEachStats> {
    let mut v = vec![];
    let mut records = parse_fastx_file(fastq_file).expect(&format!("Failed to read {fastq_file}"));
    let mut read_idx = 1;
    while let Some(Ok(record)) = records.next() {
...
pub fn chunk_records_from_fastq(fastq_file: &str, chunk: u32, need_description: bool) -> (JoinHandle<()>, Receiver<Vec<FastqRecord>>) {
    ...
    let handle = thread::spawn(
        move || {
            ...
            sender.send(fastq_records).expect("Failed to send last chunk");
        }
    );
    (handle, receiver)
}
