...
namespace myutility {
    ...
    void update_sequence_info(SequenceInfo& seq_info,
                                     int top5end_len,
    ...
    std::string check_file(const std::string& parameter, const std::string& file, bool need_directory);
    template <typename T>
    T check_number(const std::string& parameter, const std::string& number, T min, T max);

    std::string check_one_candidate(const std::string& parameter,
